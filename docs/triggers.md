...
BEGIN
    ...
    IF (TG_OP = 'UPDATE' AND OLD."mediaId" IS DISTINCT FROM NEW."mediaId") THEN
        UPDATE media_items
        SET
            "averageRating" = COALESCE((SELECT ROUND(AVG(rating), 1) FROM reviews WHERE "mediaId" = OLD."mediaId" AND rating IS NOT NULL), 0),
    ...
    UPDATE media_items
    SET
        "averageRating" = COALESCE((SELECT ROUND(AVG(rating), 1) FROM reviews WHERE "mediaId" = target_media_id AND rating IS NOT NULL), 0),
