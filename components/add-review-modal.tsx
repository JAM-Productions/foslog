...
export default function AddReviewModal() {
    ...
    const tAddReviewModal = useTranslations('AddReviewModal');
    const tMediaTypes = useTranslations('MediaTypes');

    const { isAddReviewModalOpen, setIsAddReviewModalOpen } = useAppStore();
    ...
    const options: SelectOption[] = [
        ...
        { value: 'series', label: tMediaTypes('series'), disabled: true },
            ...
            >
                <div className="w-full space-y-2 text-center">
                    <h2 className="text-2xl font-bold">
                        {tAddReviewModal('addReviewModalTitle')}
                    </h2>
                    <p className="text-muted-foreground text-center">
                        {tAddReviewModal('addReviewModalDescription')}
                    </p>
                </div>
                <div className="flex w-full flex-col gap-4 sm:flex-row">
                    <div className="">
                        <Select
                            ...
                            placeholder={tAddReviewModal('selectMediaType')}
                        />
                    </div>
                    <div className="flex-8">
                        <Input
                            disabled={!selectedMediaType}
                            placeholder={tAddReviewModal('inputMediaTitle')}
                            value={mediaTitle}
