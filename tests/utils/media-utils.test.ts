...
describe('Media Utilities', () => {
    describe('getBookGenreByIdGoogle', () => {
        ...
        it('should handle partial matches', () => {
            expect(getBookGenreByIdGoogle('Science Fiction / Cyberpunk')).toBe(
                'sciFi'
            );
            expect(getBookGenreByIdGoogle('Juvenile Fiction / Animals')).toBe(
                'kids'
            );
        });
        ...
        it('should fallback to fiction/non-fiction if no specific match found', () => {
            ...
            expect(getBookGenreByIdGoogle('General Non-Fiction')).toBe(
                'nonFiction'
            );
        });

        it('should return unknown for completely unrecognized categories', () => {
            expect(getBookGenreByIdGoogle('Unrecognized Category 123')).toBe(
                'unknown'
            );
        });
